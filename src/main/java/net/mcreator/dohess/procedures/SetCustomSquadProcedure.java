	...
	public static void execute(IWorld world, Entity entity) {
		...
		for (Entity entityiterator : new ArrayList<>(world.getPlayers())) {
			if ((entityiterator.getCachedUniqueIdString()).equals((entity.getCapability(DohessModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DohessModVariables.PlayerVariables())).ChangingRankSquad)) {
				{
					String _setval = "[{\"text\":\"[\",\"color\":\"#6E6E6E\"},{\"text\":\"" + ""
							+ ((entity instanceof PlayerEntity && ((PlayerEntity) entity).openContainer instanceof DohessModMenus.MenuAccessor) ? ((DohessModMenus.MenuAccessor) ((PlayerEntity) entity).openContainer).getMenuState(0, "squad", "") : "")
							+ "\",\"color\":\"#"
							+ ((entity instanceof PlayerEntity && ((PlayerEntity) entity).openContainer instanceof DohessModMenus.MenuAccessor)
									? ((DohessModMenus.MenuAccessor) ((PlayerEntity) entity).openContainer).getMenuState(0, "hexsquad", "")
									: "")
							+ "\"},";
				...
				{
					String _setval = (entity instanceof PlayerEntity && ((PlayerEntity) entity).openContainer instanceof DohessModMenus.MenuAccessor)
							? ((DohessModMenus.MenuAccessor) ((PlayerEntity) entity).openContainer).getMenuState(0, "prioritysquad", "")
							: "";
					entityiterator.getCapability(DohessModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
