...
public class OpenGeneralMenuProcedure {
		...
		if (entity.hasPermissionLevel(2)) {
			if (entity instanceof ServerPlayerEntity) {
				...
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("GeneralManagingMenu");
					}

					@Override
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						return new GeneralManagingMenuMenu(id, inventory, new PacketBuffer(Unpooled.buffer()).writeBlockPos(_bpos));
					}
				}, _bpos);
			}
			{
		...
		} else {
			{
				...
				if (!_ent.world.isRemote() && _ent.world.getServer() != null)
					_ent.world.getServer().getCommandManager().handleCommand(_ent.getCommandSource().withFeedbackDisabled().withPermissionLevel(4), "tellraw @s {\"text\":\"You Need /OP To Access This Command\",\"color\":\"red\"}");
			}
		}
	}
